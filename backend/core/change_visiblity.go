	...
	if newVisibility == 1 {
		...
		if err != nil {
			utils.RespondWithJSON(
				...
				map[string]string{"error": "Failed to start transaction"},
			)
			return
		}
		defer tx.Rollback()
		...
		rows, err := tx.Query(
			"SELECT follower_id FROM follow_requests WHERE following_id = ? AND status = 'pending'",
			userId,
		)
		if err != nil {
			utils.RespondWithJSON(
				w,
				http.StatusInternalServerError,
				map[string]string{"error": "Failed to fetch pending requests"},
			)
			return
		}
		defer rows.Close()
		...
		for rows.Next() {
			...
			if err := rows.Scan(&followerId); err != nil {
				utils.RespondWithJSON(
					w,
					http.StatusInternalServerError,
					map[string]string{"error": "Failed to process follow requests"},
				)
				return
			}
			...
			if err != nil {
				utils.RespondWithJSON(
					w,
					http.StatusInternalServerError,
					map[string]string{"error": "Failed to add followers"},
				)
				return
			}
		}
